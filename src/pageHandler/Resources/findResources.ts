...
export default async (interaction : ButtonInteraction): Promise<void> => {
    ...
    let resourceList: ResourceObject[] = []
    resources.forEach(resource => {
        let resourceData = new ResourceData(resource.name, resource.guildId)
        resourceData.SetData(resource)
        
        let resourceObject = new ResourceObject(resourceData, guild, interaction.user.id, interaction.user.username)

        resourceList.push(resourceObject)
    });

    const typeList = await resourceTypeSchema.findOne({guildId: guild.id})
    let tagOptions = []
    ...
    let currentFilter: ResourceObject[] = []
    ...
    collector.on('collect', async (int: BaseCommandInteraction) => {
        if(int.isSelectMenu()){
            ...
            if(currentFilter.length > 0) {
                ...
                let empty = new ResourceData('Nothing fits this filter.', guild.id);
                empty.SetDescription('Please widen your search or ask a provider for help.')
                let emptyObj = new ResourceObject(empty, guild, interaction.user.id, interaction.user.username)
