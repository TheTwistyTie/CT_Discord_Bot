...
export default class RestaurantObject {
    data: RestaurantData;
    guild: Guild;
    userID: string;
    userName: string;
    ...
    constructor(embedData: RestaurantData, guild: Guild, userID: string, userName: string) {
        this.data = embedData;
        this.guild = guild;
        this.userID = userID
    ...
    async isSaved(): Promise<boolean> {
        ...
        for (let i = 0; i < userDB.savedRestaurants.length; i++) {
            if(userDB.savedRestaurants[i] == this.data.name){
                this.#isSaved = true
                return true
            }
        }
    ...
    async save(): Promise<void> {
        ...
        while(i < userDB.savedRestaurants.length && !found) {
            if(userDB.savedRestaurants[i] == this.data.name) {
                found = true;
                this.#isSaved = false
                userDB.savedRestaurants.splice(i, 1)
            }
            i++
        }

        if(!found) {
            userDB.savedRestaurants.push(this.data.name)
            this.#isSaved = true
        }
